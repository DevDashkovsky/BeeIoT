...
func (m *MockInMemoryDB) ExistSensor(ctx context.Context, sensorID string) (bool, error) {
	return m.ExistSensorResult, m.ExistSensorError
}

func (m *MockInMemoryDB) SetSensor(ctx context.Context, sensorID string) error {
...
func (m *MockInMemoryDB) UpdateSensorTimestamp(ctx context.Context, sensorID string, timestamp int64) error {
...
func (m *MockDB) GetEmailHiveBySensorID(ctx context.Context, sensorID string) (string, string, error) {
	return m.GetEmailHiveBySensorIDResultEmail, m.GetEmailHiveBySensorIDResultHive, m.GetEmailHiveBySensorIDError
}

func (m *MockDB) NewNoise(ctx context.Context, noise httpType.NoiseLevel) error {
	return m.NewNoiseError
}

func (m *MockDB) NewTemperature(ctx context.Context, temp httpType.Temperature) error {
...
func (t *GenericMockToken) WaitTimeout(d time.Duration) bool { return t.waitTimeoutRet }
...
func (m *MockMqttClientWithToken) Publish(topic string, qos byte, retained bool, payload interface{}) mqtt.Token {
	return m.token
}
...
func TestHandleDeviceStatusTest_InvalidTopicAndBadPayload(t *testing.T) {
	...
	client.handleDeviceStatusTest(nil, msg)

	// bad payload
	msg2 := &MockMessage{topic: "/device/s1/status", payload: []byte("not json")}
	client.handleDeviceStatusTest(nil, msg2)
}
